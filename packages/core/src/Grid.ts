...
export class Grid {
  ...
  getGridPositionFromPixels(
    ...
    const innerWidth =
      rect.width - paddingLeft - (parseInt(containerStyles.paddingRight) || 0);
    const columnWidth =
      (innerWidth - this.config.gap * (this.config.columns - 1)) /
      this.config.columns;
    ...
    const column = Math.max(
      ...
      Math.min(
        this.config.columns,
        Math.round(relativeX / (columnWidth + this.config.gap)) + 1
      )
    );
  ...
  getPixelsFromGridPosition(
    ...
    const x =
      rect.left +
      paddingLeft +
      (gridPosition.column - 1) * (columnWidth + this.config.gap);
    const y =
      rect.top + paddingTop + (gridPosition.row - 1) * effectiveRowHeight;
  ...
  getGridSizeFromPixels(
    ...
    const columnSpan = Math.max(
      ...
      Math.min(
        this.config.columns,
        Math.round(size.width / (columnWidth + this.config.gap))
      )
    );
  ...
  checkGridCollision(
    ...
    for (const block of existingBlocks) {
      ...
      const existingEndColumn =
        block.gridPosition.column + block.gridSize.columnSpan - 1;
      const existingEndRow =
        block.gridPosition.row + block.gridSize.rowSpan - 1;

      const horizontalOverlap = !(
        newPosition.column > existingEndColumn ||
        newEndColumn < block.gridPosition.column
      );
  ...
  findAvailablePosition(
    ...
    for (let row = 1; row <= 100; row++) {
      for (
        let column = 1;
        column <= this.config.columns - gridSize.columnSpan + 1;
        column++
      ) {
